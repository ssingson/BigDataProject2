...
if __name__ == "__main__":
    ...
    lr = LogisticRegression(featuresCol = 'features', labelCol = 'grade', maxIter = 5)
    lrModel = lr.fit(train)
    predictions = lrModel.transform(test)

    #Compute and print the accuracy
    accuracy = predictions.filter(predictions.grade == predictions.prediction).count() / float(predictions.count())
    print("The accuracy of the dataset of a Logistic Regression Classifier is %s." % accuracy)
